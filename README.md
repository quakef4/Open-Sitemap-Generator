...
**Dynamic XML sitemap generator for WordPress** with automatic pagination, IndexNow support (Bing/Yandex), and Google notification. Optimized for WooCommerce stores with tens of thousands of products.
...
- **Full admin dashboard** — Statistics, queue management, activity log, one-click actions
...
|--------|--------|--------|
| **Google** | `robots.txt` + Search Console | ✅ Automatic |
| **Bing** | IndexNow (queued batch) | ✅ Instant |
| **Yandex** | IndexNow (shared from Bing) | ✅ Instant |

> **Note:** Google does NOT support IndexNow. The primary method for Google is `robots.txt` + manual Search Console registration.
...
```
open-sitemap-generator/
├── open-sitemap-generator.php    # Main plugin file
├── includes/
...
Developed by [Infobit snc](https://infobitcomputer.it) — IT equipment and e-commerce solutions since Nerviano, Italy.
